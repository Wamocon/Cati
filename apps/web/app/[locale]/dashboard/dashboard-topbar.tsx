...
export function DashboardTopbar() {
  ...
  return (
    <header data-testid="dashboard-topbar" className="sticky top-0 z-30 border-b border-border/70 bg-background/86 backdrop-blur-xl">
      <div className="flex min-h-16 min-w-0 items-center justify-between gap-2 px-4 pl-16 md:gap-3 md:px-6 md:pl-6 lg:px-8">
        ...
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
          ...
          <details className="group relative">
            <summary
              aria-label={`${userDisplayName} - ${roleLabel}`}
              className="flex h-10 cursor-pointer list-none items-center gap-2 rounded-full border border-border bg-card px-2 text-xs font-bold text-foreground shadow-sm transition hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary sm:px-3 [&::-webkit-details-marker]:hidden"
              title={userDisplayName}
            >
              <ShieldCheck className="h-4 w-4 text-primary" />
              <span className="sr-only">{userDisplayName} - {roleLabel}</span>
