...
test.describe("Landing page journey", () => {
  ...
  test("home page renders all sections", async ({ page }, testInfo) => {
    ...
    // Problems bento
    const problemCards = page.locator("[data-testid='problem-bento'] > div > div.grid > div")
    ...
    await expect(page.getByText("Oturum izni kuralları sıkılaştı")).toBeVisible()
    await screenshot(page, testInfo, "03-problems")

    // Solution grid
    await scrollToSection(page, "[data-testid='solution-grid']")
    const solutionCards = page.locator("[data-testid='solution-grid'] > div > div.grid > div")
    await expect(solutionCards).toHaveCount(12)
    await expect(page.getByText("EİDS yetkilendirme takibi ve son kullanma hatırlatıcıları")).toBeVisible()
    await screenshot(page, testInfo, "04-solution")

    // Compliance section
    await scrollToSection(page, "[data-testid='compliance-features']")
    const complianceCards = page.locator("[data-testid='compliance-features'] > div > div.grid > div")
    ...
    // How it works
    await scrollToSection(page, "section:has-text('Nasıl çalışırız')")
    await expect(page.getByText("Ataberk Estate'e başvurun")).toBeVisible()
    await screenshot(page, testInfo, "07-how-it-works")

    // CTA
    await scrollToSection(page, "section:has-text('Emlak operasyonunuzu 2026 kurallarına göre yönetin')")
    ...
    await screenshot(page, testInfo, "10-full-page", { fullPage: true })
